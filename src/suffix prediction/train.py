...
class Trainer:
    ...
    def _safe_add_scalars(self, tag, scalar_dict, step):
...
class KunklerTrainer(Trainer):
    def __init__(self,
        ...
        - gradnorm_values: deprecated and ignored (kept for backward compatibility).
        """
...
class TaymouriTraining(Trainer):
    def __init__(self,
                 device,
                 model,
                 data_train,
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if self.writer is not None:
                self._safe_add_scalars("Taymouri Losses", {
                    "Generator Train": epoch_gen_loss,
                    "Discriminator Train": epoch_disc_loss,
                    "Validation CE": val_loss,
                }, epoch + 1)

                self._safe_add_scalars("Taymouri Metrics", {
                    "Beam Token Accuracy": val_acc,
                    "Teacher Forcing Ratio": self.teacher_forcing_ratio,
                }, epoch + 1)
