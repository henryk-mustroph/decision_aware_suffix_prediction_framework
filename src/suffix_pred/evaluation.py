...
class DLSEvaluation:
    ...
    def collect_inference_outputs(
        ...
        if mode == "probabilistic":
            outputs = self.collect_probabilistic_inference_samples(
                random_order=random_order,
    ...
    def evaluate_from_inference_outputs(
        self,
        outputs: list[dict],
    ...
    def sample_suffix_predictions(
        ...
        if max_examples <= 0:
            return pd.DataFrame(
                columns=["case_id", "prefix_len", "prefix", "target_suffix", "predicted_suffixes", "mode"]
            )

        outputs = self.collect_inference_outputs(
            mode=mode,
            random_order=random_order,
        ...
        for row in outputs[:max_examples]:
            ...
            if (
                mode == "probabilistic"
                and max_probabilistic_suffixes is not None
                and max_probabilistic_suffixes >= 0
            ):
    ...
    def sample_suffix_predictions_all_modes(
        self,
        random_order: bool = False,
        max_examples_per_mode: int = 10,
        max_probabilistic_suffixes: Optional[int] = 5,
        modes: Optional[list[DecodeMode]] = None,
    ) -> dict[str, pd.DataFrame]:
        """
        Return example prefix/target/prediction tables for multiple decode modes.
        ...
        for mode in selected_modes:
            results[mode] = self.sample_suffix_predictions(
                mode=mode,
                random_order=random_order,
    ...
    def dls_per_prefix_length(results_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if has_probabilistic_bounds:
            grouped = (
                results_df.groupby("prefix_len", as_index=False)
                .agg(
                    dls=("dls", "mean"),
